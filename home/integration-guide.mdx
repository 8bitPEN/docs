...
```bash
npm install @exponent-labs/exponent-sdk @solana/web3.js @coral-xyz/anchor
```
...
async function example1_CompleteLifecycle() {
  ...
  let sig = await sendAndConfirmTransaction(connection, tx, [wallet.payer], {
  ...
  if (!yieldPositionAccount) {
    ...
    sig = await sendAndConfirmTransaction(connection, tx, [wallet.payer]);
...
async function example2_OrderbookYieldEarning() {
  ...
  const postOfferIx = orderbook.ixPostOffer({
    ...
    offerOption: exponentOrderbook.offerOptions("FillOrKill", [false]), // Allow partial fills
  ...
  const withdrawIx = await orderbook.ixWrapperWithdrawFunds({
    trader: user,
    mintSy: vault.mintSy,
    ptAmount: exponentOrderbook.amount("All"),
    ytAmount: exponentOrderbook.amount("All"),
    syAmount: exponentOrderbook.amount("All"),
  });
...
async function example3_VirtualOffers() {
  ...
  const { ix: marketOfferIx, setupIxs: marketSetupIxs } = await orderbook.ixWrapperMarketOffer({
    ...
    offerType: exponentOrderbook.OfferType.BuyYt, // We're buying YT (taking SellYT offers)
...
async function example4_CLMMLiquidityProvision() {
  ...
  sig = await sendAndConfirmTransaction(
    connection,
    tx,
    [wallet.payer, depositResult.signers] // signers is the generated LP position keypair
  );
